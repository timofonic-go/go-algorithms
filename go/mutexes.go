		...
		go func() {
			...
			for {
				...
				*/
				...
				// In order to ensure that this goroutine doesn’t starve the scheduler, we explicitly yield after each operation with runtime.Gosched().
				// This yielding is handled automatically with e.g. every channel operation and for blocking calls like time.Sleep, but in this case we need to do it manually.
				// todo: find information why we need to do this manually?
				runtime.Gosched()
			}
